...
// source: http://www.cs.ucr.edu/~ycao009/Teaching_files/Lab5_Solution.cc
...
// n1 n0
/*
 * Consider a Wifi access point (AP) and a base station (STA) both static.
 ...
 * Given below is a code to simulate the aforementioned scenario with ns3.
 ...
 * Increase the value of xDistance in the code and find out the maximum distance up
 * to which two way communication is possible. This can be verified from the output
 * of the code, which will show the STA has received reply from the AP
 * (indicated by IP address). Node #0 is the AP, #1 is a base station
 * #1 sends UDP echo message to the AP; AP sends a UDP response back to the node
