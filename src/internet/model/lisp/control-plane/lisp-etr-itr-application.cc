...
namespace ns3
{
  ...
  LispEtrItrApplication::GetTypeId (void)
  {
	...
	LispEtrItrApplication> ().AddAttribute (
  ...
  void
  LispEtrItrApplication::SetMapServerAddresses (
  ...
  void
  LispEtrItrApplication::SetMapTables (Ptr<MapTables> mapTablesV4,
  ...
  void
  LispEtrItrApplication::AddMapResolverLoc (Ptr<Locator> locator)
  {
    m_mapResolverRlocs.push_back (locator);
  }

  void
  LispEtrItrApplication::DoDispose (void)
  {
    Application::DoDispose ();
  }

  void
  LispEtrItrApplication::StartApplication (void)
  {
    if (m_lispMappingSocket == 0)
      {
	NS_LOG_DEBUG("Trying to create a lisp mapping socket (between lisp and xTR) which connects to lispProto");
	...
	if (Ipv4Address::IsMatchingType (m_mapServerAddress.front ()))
	  {
	    ...
	    m_socket->Connect (
		InetSocketAddress (
		    Ipv4Address::ConvertFrom (m_mapServerAddress.front ()),
		    m_peerPort));
	  }
	else if (Ipv6Address::IsMatchingType (m_mapServerAddress.front ()))
	  {
	    ...
	    m_socket->Connect (
		Inet6SocketAddress (
		    Ipv6Address::ConvertFrom (m_mapServerAddress.front ()),
		    m_peerPort));
	  }
      }
  ...
  void
  LispEtrItrApplication::StopApplication (void)
  {
    NS_LOG_FUNCTION(this);
  ...
  void
  LispEtrItrApplication::ScheduleTransmit (Time dt)
  {
  ...
  void
  LispEtrItrApplication::SendMapRegisters (void)
  {
    NS_LOG_FUNCTION(this);
  ...
  void
  LispEtrItrApplication::SendToLisp (Ptr<Packet> packet)
  {
    NS_LOG_FUNCTION(this);
  ...
  void
  LispEtrItrApplication::HandleReadControlMsg (Ptr<Socket> socket)
  {
    NS_LOG_FUNCTION(this);
    Ptr<Packet> packet;
    Address from;
	...
	if (msg_type == MapRequestMsg::GetMsgType ())
	  {
		...
		NS_LOG_DEBUG(
		    "Receive a SMR on ETR from " << Ipv4Address::ConvertFrom (requestMsg->GetItrRlocAddrIp ())<<". Prepare an invoked Map Request Message.");
...
//		    LispEtrItrApplication::GenerateMapRequest ();
		...
		if (Ipv4Address::IsMatchingType (itrAddress))
		  {
		    requestMsg->SetItrRlocAddrIp (itrAddress);
		  }
		else
		  {
		    requestMsg->SetItrRlocAddrIpv6 (itrAddress);
		  }
		// TODO:Actually, we also choose a new nonce number. (RFC6830)
		requestMsg->Serialize (newBuf);
		reactedPacket = Create<Packet> (newBuf, 256);
...
// As a first step, we send SMR to all xTRs in the cache databases.
  ...
  LispEtrItrApplication::SendSmrMsg ()
  {
	...
	Send (packetSmrMsg);
	NS_LOG_DEBUG(
	    "A SMR message has been sent to xTR "<<dstRlocAddr);
      }

  }

  void
  LispEtrItrApplication::HandleMapSockRead (Ptr<Socket> lispMappingSocket)
  {
    NS_LOG_FUNCTION(this);
    Ptr<Packet> packet;
    Address from;
  ...
  LispEtrItrApplication::GenerateMapSocketAddMsgHeader (
  ...
  LispEtrItrApplication::GenerateMapSocketAddMsgBody (Ptr<MapReplyMsg> replyMsg)
  {
  ...
  Ptr<MapRegisterMsg>
  LispEtrItrApplication::GenerateMapRegister (Ptr<MapEntry> mapEntry)
  {
  ...
  Ptr<MapReplyMsg>
  LispEtrItrApplication::GenerateMapReply (Ptr<MapRequestMsg> requestMsg)
  {
  ...
  Ptr<MapReplyMsg>
  LispEtrItrApplication::GenerateMapReply4ChangedMapping (Ptr<MapRequestMsg> requestMsg)
  {
    /**
    ...
    std::list<Ptr<MapEntry> > mapEntries;
    NS_ASSERT_MSG(
	m_mapTablesV4->GetNMapEntriesLispDataBase () + m_mapTablesV6->GetNMapEntriesLispDataBase () != 0,
  ...
  void
  LispEtrItrApplication::Send (Ptr<Packet> packet)
  {
    NS_LOG_FUNCTION(this);
  ...
  Address
  LispEtrItrApplication::GetLocalAddress (Address address)
  {
	...
	if (route)
	  {
	    ...
	    Ipv4Address ipv4SrcAddress =
		(m_node->GetObject<Ipv4> ()->GetAddress (interface, 0)).GetLocal ();
	...
	if (route)
	  {
		...
		(m_node->GetObject<Ipv6> ()->GetAddress (interface, 0)).GetAddress ();
  ...
  bool
  LispEtrItrApplication::IsInRequestList (Ptr<EndpointId> eid) const
  {
  ...
  bool
  LispEtrItrApplication::IsInRequestCounter (Ptr<EndpointId> eid) const
  {
  ...
  uint8_t
  LispEtrItrApplication::GetRequestCount (Ptr<EndpointId> eid)
  {
    return m_requestCounter.at (eid);
  }

  void
  LispEtrItrApplication::AddInMapReqList (Ptr<EndpointId> eid,
  ...
  void
  LispEtrItrApplication::DeleteFromMapReqList (Ptr<EndpointId> eid)
  {
