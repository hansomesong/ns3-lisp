...
TypeId Ipv4L3Protocol::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::Ipv4L3Protocol").SetParent<Ipv4>().SetGroupName(
					"Internet").AddConstructor<Ipv4L3Protocol>().AddAttribute(
					"DefaultTtl", "The TTL value set by default on "
					...
					"ns3::Ipv4L3Protocol::DropTracedCallback").AddAttribute(
					"InterfaceList",
...
void Ipv4L3Protocol::Insert(Ptr<IpL4Protocol> protocol) {
	NS_LOG_FUNCTION(this << protocol);
	L4ListKey_t key = std::make_pair(protocol->GetProtocolNumber(), -1);
	if (m_protocols.find(key) != m_protocols.end()) {
		NS_LOG_WARN(
				"Overwriting default protocol " << int(protocol->GetProtocolNumber ()));
	}
...
void Ipv4L3Protocol::Insert(Ptr<IpL4Protocol> protocol,
	...
	L4ListKey_t key = std::make_pair(protocol->GetProtocolNumber(),
			interfaceIndex);
	if (m_protocols.find(key) != m_protocols.end()) {
		NS_LOG_WARN(
				"Overwriting protocol " << int(protocol->GetProtocolNumber ()) << " on interface " << int(interfaceIndex));
	}
	m_protocols[key] = protocol;
}

void Ipv4L3Protocol::Remove(Ptr<IpL4Protocol> protocol) {
	NS_LOG_FUNCTION(this << protocol);
	...
	if (iter == m_protocols.end()) {
		NS_LOG_WARN(
				"Trying to remove an non-existent default protocol " << int(protocol->GetProtocolNumber ()));
	} else {
...
void Ipv4L3Protocol::Remove(Ptr<IpL4Protocol> protocol,
	...
	if (iter == m_protocols.end()) {
		NS_LOG_WARN(
				"Trying to remove an non-existent protocol " << int(protocol->GetProtocolNumber ()) << " on interface " << int(interfaceIndex));
	} else {
		m_protocols.erase(key);
	}
}

Ptr<IpL4Protocol> Ipv4L3Protocol::GetProtocol(int protocolNumber) const {
	NS_LOG_FUNCTION(this << protocolNumber);

	return GetProtocol(protocolNumber, -1);
}

Ptr<IpL4Protocol> Ipv4L3Protocol::GetProtocol(int protocolNumber,
...
Ptr<Socket> Ipv4L3Protocol::CreateRawSocket(void) {
	NS_LOG_FUNCTION(this);
...
void Ipv4L3Protocol::DeleteRawSocket(Ptr<Socket> socket) {
	NS_LOG_FUNCTION(this << socket);
	for (SocketList::iterator i = m_sockets.begin(); i != m_sockets.end();
			++i) {
		if ((*i) == socket) {
			m_sockets.erase(i);
			return;
...
void Ipv4L3Protocol::SetRoutingProtocol(
...
Ptr<Ipv4RoutingProtocol> Ipv4L3Protocol::GetRoutingProtocol(void) const {
	NS_LOG_FUNCTION(this);
	return m_routingProtocol;
}

void Ipv4L3Protocol::DoDispose(void) {
	NS_LOG_FUNCTION(this);
	for (L4List_t::iterator i = m_protocols.begin(); i != m_protocols.end();
			++i) {
		i->second = 0;
	}
	m_protocols.clear();
	...
	for (MapFragments_t::iterator it = m_fragments.begin();
			it != m_fragments.end(); it++) {
		it->second = 0;
	}

	for (MapFragmentsTimers_t::iterator it = m_fragmentsTimers.begin();
			it != m_fragmentsTimers.end(); it++) {
...
void Ipv4L3Protocol::SetupLoopback(void) {
	NS_LOG_FUNCTION(this);
	...
	Ipv4InterfaceAddress ifaceAddr = Ipv4InterfaceAddress(
			Ipv4Address::GetLoopback(), Ipv4Mask::GetLoopback());
...
void Ipv4L3Protocol::SetDefaultTtl(uint8_t ttl) {
	NS_LOG_FUNCTION(this << static_cast<uint32_t> (ttl));
	m_defaultTtl = ttl;
}

uint32_t Ipv4L3Protocol::AddInterface(Ptr<NetDevice> device) {
	...
	m_node->RegisterProtocolHandler(
			MakeCallback(&TrafficControlLayer::Receive, tc),
			Ipv4L3Protocol::PROT_NUMBER, device);
	m_node->RegisterProtocolHandler(
			MakeCallback(&TrafficControlLayer::Receive, tc),
	...
	tc->RegisterProtocolHandler(
			MakeCallback(&ArpL3Protocol::Receive,
...
uint32_t Ipv4L3Protocol::AddIpv4Interface(Ptr<Ipv4Interface> interface) {
...
Ptr<Ipv4Interface> Ipv4L3Protocol::GetInterface(uint32_t index) const {
...
uint32_t Ipv4L3Protocol::GetNInterfaces(void) const {
	NS_LOG_FUNCTION(this);
	return m_interfaces.size();
}

int32_t Ipv4L3Protocol::GetInterfaceForAddress(Ipv4Address address) const {
	NS_LOG_FUNCTION(this << address);
	int32_t interface = 0;
	for (Ipv4InterfaceList::const_iterator i = m_interfaces.begin();
			i != m_interfaces.end(); i++, interface++) {
...
int32_t Ipv4L3Protocol::GetInterfaceForPrefix(Ipv4Address address,
		Ipv4Mask mask) const {
	...
	for (Ipv4InterfaceList::const_iterator i = m_interfaces.begin();
			i != m_interfaces.end(); i++, interface++) {
		for (uint32_t j = 0; j < (*i)->GetNAddresses(); j++) {
			if ((*i)->GetAddress(j).GetLocal().CombineMask(mask)
...
int32_t Ipv4L3Protocol::GetInterfaceForDevice(
		Ptr<const NetDevice> device) const {
	NS_LOG_FUNCTION(this << device);

	Ipv4InterfaceReverseContainer::const_iterator iter =
			m_reverseInterfacesContainer.find(device);
	...
	if (GetWeakEsModel())  // Check other interfaces
	{
		for (uint32_t j = 0; j < GetNInterfaces(); j++) {
			...
			for (uint32_t i = 0; i < GetNAddresses(j); i++) {
				...
				if (address == iaddr.GetLocal()) {
					NS_LOG_LOGIC(
							"For me (destination " << address << " match) on another interface");
					return true;
				}
...
void Ipv4L3Protocol::Receive(Ptr<NetDevice> device, Ptr<const Packet> p,
	...
	NS_LOG_LOGIC(
			"Packet from " << from << " received on node " << m_node->GetId ());
	...
	if (arpCache) {
		...
		if (entry) {
			...
			std::list<ArpCache::Entry *> entryList = arpCache->LookupInverse(
					from);
	...
	NS_ASSERT_MSG(m_routingProtocol != 0,
	...
	if (!m_routingProtocol->RouteInput(packet, ipHeader, device,
		...
		m_dropTrace(ipHeader, packet, DROP_NO_ROUTE, m_node->GetObject<Ipv4>(),
				interface);
	}
}

Ptr<Icmpv4L4Protocol> Ipv4L3Protocol::GetIcmp(void) const {
	NS_LOG_FUNCTION(this);
	Ptr<IpL4Protocol> prot = GetProtocol(
...
bool Ipv4L3Protocol::IsUnicast(Ipv4Address ad) const {
	...
	if (ad.IsBroadcast() || ad.IsMulticast()) {
		...
		for (uint32_t ifaceIndex = 0; ifaceIndex < GetNInterfaces();
				ifaceIndex++) {
			for (uint32_t j = 0; j < GetNAddresses(ifaceIndex); j++) {
				Ipv4InterfaceAddress ifAddr = GetAddress(ifaceIndex, j);
				NS_LOG_LOGIC(
						"Testing address " << ad << " with subnet-directed broadcast " << ifAddr.GetBroadcast ());
...
bool Ipv4L3Protocol::IsUnicast(Ipv4Address ad, Ipv4Mask interfaceMask) const {
	NS_LOG_FUNCTION(this << ad << interfaceMask);
	return !ad.IsMulticast() && !ad.IsSubnetDirectedBroadcast(interfaceMask);
}

void Ipv4L3Protocol::SendWithHeader(Ptr<Packet> packet, Ipv4Header ipHeader,
		Ptr<Ipv4Route> route) {
...
void Ipv4L3Protocol::CallTxTrace(const Ipv4Header & ipHeader,
		Ptr<Packet> packet, Ptr<Ipv4> ipv4, uint32_t interface) {
	Ptr<Packet> packetCopy = packet->Copy();
	packetCopy->AddHeader(ipHeader);
	m_txTrace(packetCopy, ipv4, interface);
}

void Ipv4L3Protocol::Send(Ptr<Packet> packet, Ipv4Address source,
	...
	if (lispOverIpv4 != 0 and nbEntriesDB != 0) {
		...
		if (route) {
			...
			if (m_routingProtocol != 0) {
				newRoute = m_routingProtocol->RouteOutput(packet, innerIpHeader,
						oif, errno_);
			} else {
	...
	if (destination.IsBroadcast() || destination.IsLocalMulticast()) {
		...
		ipHeader = BuildHeader(source, destination, protocol, packet->GetSize(),
				ttl, tos, mayFragment);
		...
		for (Ipv4InterfaceList::iterator ifaceIter = m_interfaces.begin();
			...
			for (uint32_t index = 0; index < outInterface->GetNAddresses();
					index++) {
			...
			if (sendIt) {
				...
				CallTxTrace(ipHeader, packetCopy, m_node->GetObject<Ipv4>(),
						ifaceIndex);
	...
	Ptr<Ipv4Route> newRoute;
	if (m_routingProtocol != 0) {
		newRoute = m_routingProtocol->RouteOutput(packet, ipHeader, oif,
				errno_);
	} else {
		NS_LOG_ERROR("Ipv4L3Protocol::Send: m_routingProtocol == 0");
	}
...
Ipv4Header Ipv4L3Protocol::BuildHeader(Ipv4Address source,
		Ipv4Address destination, uint8_t protocol, uint16_t payloadSize,
		uint8_t ttl, uint8_t tos, bool mayFragment) {
	NS_LOG_FUNCTION(
...
void Ipv4L3Protocol::SendRealOut(Ptr<Ipv4Route> route, Ptr<Packet> packet,
	...
	NS_LOG_LOGIC(
			"Send via NetDevice ifIndex " << outDev->GetIfIndex () << " ipv4InterfaceIndex " << interface);

	if (!route->GetGateway().IsEqual(Ipv4Address("0.0.0.0"))) {
		if (outInterface->IsUp()) {
			NS_LOG_LOGIC("Send to gateway " << route->GetGateway ());
			if (packet->GetSize() + ipHeader.GetSerializedSize()
					> outInterface->GetDevice()->GetMtu()) {
				std::list<Ipv4PayloadHeaderPair> listFragments;
				DoFragmentation(packet, ipHeader,
						outInterface->GetDevice()->GetMtu(), listFragments);
				for (std::list<Ipv4PayloadHeaderPair>::iterator it =
						listFragments.begin(); it != listFragments.end();
						it++) {
					CallTxTrace(it->second, it->first,
							m_node->GetObject<Ipv4>(), interface);
					outInterface->Send(it->first, it->second,
							route->GetGateway());
				}
			} else {
				CallTxTrace(ipHeader, packet, m_node->GetObject<Ipv4>(),
						interface);
...
void Ipv4L3Protocol::LocalDeliver(Ptr<const Packet> packet, Ipv4Header const&ip,
		uint32_t iif) {
	...
	NS_LOG_DEBUG(
			"OK on node " << m_node->GetId ()<<"-> No decapsulation when local deliver");
	...
	if (protocol != 0) {
		...
		enum IpL4Protocol::RxStatus status = protocol->Receive(p, ipHeader,
				GetInterface(iif));
		switch (status) {
		...
		case IpL4Protocol::RX_ENDPOINT_UNREACH:
			...
			for (uint32_t i = 0; i < GetNAddresses(iif); i++) {
				...
				if (addr.GetLocal().CombineMask(addr.GetMask())
						...
						&& ipHeader.GetDestination().IsSubnetDirectedBroadcast(
								addr.GetMask())) {
					subnetDirected = true;
				}
			}
...
bool Ipv4L3Protocol::AddAddress(uint32_t i, Ipv4InterfaceAddress address) {
...
Ipv4InterfaceAddress Ipv4L3Protocol::GetAddress(uint32_t interfaceIndex,
...
uint32_t Ipv4L3Protocol::GetNAddresses(uint32_t interface) const {
	NS_LOG_FUNCTION(this << interface);
	Ptr<Ipv4Interface> iface = GetInterface(interface);
	return iface->GetNAddresses();
}

bool Ipv4L3Protocol::RemoveAddress(uint32_t i, uint32_t addressIndex) {
...
Ipv4Address Ipv4L3Protocol::SourceAddressSelection(uint32_t interfaceIdx,
	...
	for (uint32_t i = 0; i < GetNAddresses(interfaceIdx); i++) {
		...
		if (test.GetLocal().CombineMask(test.GetMask())
				== dest.CombineMask(test.GetMask())) {
...
Ipv4Address Ipv4L3Protocol::SelectSourceAddress(Ptr<const NetDevice> device,
	...
	if (device != 0) {
		...
		for (uint32_t j = 0; j < GetNAddresses(i); j++) {
			...
			if (iaddr.GetScope() > scope)
				continue;
			if (dst.CombineMask(iaddr.GetMask())
					== iaddr.GetLocal().CombineMask(iaddr.GetMask())) {
	...
	NS_LOG_WARN(
			"Could not find source address for " << dst << " and scope " << scope << ", returning 0");
	return addr;
}

void Ipv4L3Protocol::SetMetric(uint32_t i, uint16_t metric) {
	NS_LOG_FUNCTION(this << i << metric);
	Ptr<Ipv4Interface> interface = GetInterface(i);
	interface->SetMetric(metric);
}

uint16_t Ipv4L3Protocol::GetMetric(uint32_t i) const {
	NS_LOG_FUNCTION(this << i);
	Ptr<Ipv4Interface> interface = GetInterface(i);
	return interface->GetMetric();
}

uint16_t Ipv4L3Protocol::GetMtu(uint32_t i) const {
	NS_LOG_FUNCTION(this << i);
	Ptr<Ipv4Interface> interface = GetInterface(i);
	return interface->GetDevice()->GetMtu();
}

bool Ipv4L3Protocol::IsUp(uint32_t i) const {
	NS_LOG_FUNCTION(this << i);
	Ptr<Ipv4Interface> interface = GetInterface(i);
	return interface->IsUp();
}

void Ipv4L3Protocol::SetUp(uint32_t i) {
	NS_LOG_FUNCTION(this << i);
	Ptr<Ipv4Interface> interface = GetInterface(i);
	...
	if (interface->GetDevice()->GetMtu() >= 68) {
		...
		NS_LOG_LOGIC(
				"Interface " << int(i) << " is set to be down for IPv4. Reason: not respecting minimum IPv4 MTU (68 octects)");
	}
}

void Ipv4L3Protocol::SetDown(uint32_t ifaceIndex) {
...
bool Ipv4L3Protocol::IsForwarding(uint32_t i) const {
	NS_LOG_FUNCTION(this << i);
	Ptr<Ipv4Interface> interface = GetInterface(i);
...
void Ipv4L3Protocol::SetForwarding(uint32_t i, bool val) {
	NS_LOG_FUNCTION(this << i);
	Ptr<Ipv4Interface> interface = GetInterface(i);
	interface->SetForwarding(val);
}

Ptr<NetDevice> Ipv4L3Protocol::GetNetDevice(uint32_t i) {
	NS_LOG_FUNCTION(this << i);
	return GetInterface(i)->GetDevice();
}

void Ipv4L3Protocol::SetIpForward(bool forward) {
...
bool Ipv4L3Protocol::GetIpForward(void) const {
	NS_LOG_FUNCTION(this);
	return m_ipForward;
}

void Ipv4L3Protocol::SetWeakEsModel(bool model) {
	NS_LOG_FUNCTION(this << model);
	m_weakEsModel = model;
}

bool Ipv4L3Protocol::GetWeakEsModel(void) const {
	NS_LOG_FUNCTION(this);
...
void Ipv4L3Protocol::RouteInputError(Ptr<const Packet> p,
	...
	NS_LOG_LOGIC(
			"Route input failure-- dropping packet to " << ipHeader << " with errno " << sockErrno);
	m_dropTrace(ipHeader, p, DROP_ROUTE_ERROR, m_node->GetObject<Ipv4>(), 0);

	// \todo Send an ICMP no route.
}

void Ipv4L3Protocol::DoFragmentation(Ptr<Packet> packet,
		const Ipv4Header & ipv4Header, uint32_t outIfaceMtu,
		std::list<Ipv4PayloadHeaderPair>& listFragments) {
	...
	uint32_t fragmentSize = (outIfaceMtu - ipv4Header.GetSerializedSize())
			& ~uint32_t(0x7);
	...
	do {
		...
		NS_LOG_LOGIC(
				"Fragment creation - " << offset << ", " << currentFragmentablePartSize);
		Ptr<Packet> fragment = p->CreateFragment(offset,
				currentFragmentablePartSize);
		NS_LOG_LOGIC(
				"Fragment created - " << offset << ", " << fragment->GetSize ());
		...
		NS_LOG_LOGIC(
				"Fragment check - " << fragmentHeader.GetFragmentOffset ());
		...
		listFragments.push_back(
				Ipv4PayloadHeaderPair(fragment, fragmentHeader));
...
bool Ipv4L3Protocol::ProcessFragment(Ptr<Packet>& packet, Ipv4Header& ipHeader,
		uint32_t iif) {
	NS_LOG_FUNCTION(this << packet << ipHeader << iif);

	uint64_t addressCombination = uint64_t(ipHeader.GetSource().Get()) << 32
			| uint64_t(ipHeader.GetDestination().Get());
	uint32_t idProto = uint32_t(ipHeader.GetIdentification()) << 16
			| uint32_t(ipHeader.GetProtocol());
	...
	if (it == m_fragments.end()) {
		...
		m_fragmentsTimers[key] = Simulator::Schedule(
				...
				&Ipv4L3Protocol::HandleFragmentsTimeout, this, key, ipHeader,
				iif);
	} else {
		fragments = it->second;
	}
	...
	NS_LOG_LOGIC(
			"Adding fragment - Size: " << packet->GetSize ( ) << " - Offset: " << (ipHeader.GetFragmentOffset ()));

	fragments->AddFragment(p, ipHeader.GetFragmentOffset(),
			!ipHeader.IsLastFragment());
	...
	if (fragments->IsEntire()) {
		...
		if (m_fragmentsTimers[key].IsRunning()) {
			NS_LOG_LOGIC(
					"Stopping WaitFragmentsTimer at " << Simulator::Now ().GetSeconds () << " due to complete packet");
...
void Ipv4L3Protocol::Fragments::AddFragment(Ptr<Packet> fragment,
		uint16_t fragmentOffset, bool moreFragment) {
	...
	m_fragments.insert(it,
			std::pair<Ptr<Packet>, uint16_t>(fragment, fragmentOffset));
}

bool Ipv4L3Protocol::Fragments::IsEntire() const {
	NS_LOG_FUNCTION(this);
	...
	if (ret) {
		...
		for (std::list<std::pair<Ptr<Packet>, uint16_t> >::const_iterator it =
			...
			NS_LOG_LOGIC(
					"Checking overlaps " << lastEndOffset << " - " << it->second);
...
Ptr<Packet> Ipv4L3Protocol::Fragments::GetPacket() const {
	NS_LOG_FUNCTION(this);
...
Ptr<Packet> Ipv4L3Protocol::Fragments::GetPartialPacket() const {
	NS_LOG_FUNCTION(this);
...
void Ipv4L3Protocol::HandleFragmentsTimeout(std::pair<uint64_t, uint32_t> key,
	...
	m_dropTrace(ipHeader, packet, DROP_FRAGMENT_TIMEOUT,
			m_node->GetObject<Ipv4>(), iif);
